...
perr_t PDC_Server_metadata_index_create(metadata_index_create_in_t *in, metadata_index_create_out_t *out){
    ...
    uint32_t obj_server_id = in->server_id;
    uint32_t obj_locator = in->key_hash;

    uint64_t obj_id = in->obj_id;

    /*****
        obj_locator is a hash string generated by a hashing function with the name of the object as input.
        |DART|
            dart_leaf ->|Server_ID that has the index to lookup
                        |ART prefix/suffix|
                            art_leaf -> |hashtable(server_id, obj_locator)
                                        |Data_type of matched key.
                                        |Root of value index|
                                            |ART/B-Tree/Skiplist|
                                                value_leaf -> |hashtable(server_id, obj_locator)
        Each leaf node will match an exact attribute name, and there we store a value index pointer.
    *****/

    // If the global key tree is not there, we have to initialize it.
    // Otherwise, we have nowhere to start from.
    if (art_key_prefix_tree_g == NULL) {
        art_tree_init(art_key_prefix_tree_g);
    }
    // Search through the index to see if the key has been indexed.
    // Since each key is the complete key for an attribute, we just search through the tree.
    index_leaf_content* leaf_cnt =  (index_leaf_content*)art_search(art_key_prefix_tree_g,
        (unsigned char *)key, strlen(key));

    // Every leaf node of the key index contains a pointer to the
    // obj_id hash table and also a pointer to the value tree.
    hg_hash_table_t *name_prefix_obj_locator_table = NULL;
    art_tree *value_tree = NULL;
    hg_hash_table_t *value_obj_locator_table = NULL;
    // Case 1: The key has not been indexed.
        // Step 1: build index for value first
        // substep 1: prepare leaf content first
    if (leaf_cnt == NULL) {
        // Initialize the Hash Table.
        name_prefix_obj_locator_table = hg_hash_table_new(PDC_Index_server_id_hash, PDC_Index_server_id_equal);
        if (name_prefix_obj_locator_table == NULL) {
            printf("==PDC_SERVER: name_prefix_obj_locator_table init error! Exit...\n");
            exit(-1);
        }
        hg_hash_table_register_free_functions(name_prefix_obj_locator_table, PDC_Index_server_id_free, PDC_Index_obj_locator_free);
        // Currently, the pointer to actual metadata is unavailable.
        hg_hash_table_insert(name_prefix_obj_locator_table, obj_server_id, obj_locator);

        // Initialize value_obj_locator_table
        value_obj_locator_table = hg_hash_table_new(PDC_Index_server_id_hash, PDC_Index_server_id_equal);
        if (value_obj_locator_table == NULL) {
            printf("==PDC_SERVER: name_prefix_obj_locator_table init error! Exit...\n");
            exit(-1);
        }
        hg_hash_table_register_free_functions(value_obj_locator_table, PDC_Index_server_id_free, PDC_Index_obj_locator_free);
        // Currently, the pointer to actual metadata is unavailable.
        hg_hash_table_insert(value_obj_locator_table, obj_server_id, obj_locator);

        // Fulfill value tree
        art_tree_init(value_tree);
        art_insert(value_tree, (unsigned char*)str_value,
        strlen(str_value), (void*)value_obj_locator_table);

        // fulfill leaf_node_cnt of prefix tree.
        leaf_cnt = (index_leaf_content *)malloc(sizeof(index_leaf_content));
        leaf_cnt->obj_id_table = name_prefix_obj_locator_table;
        leaf_cnt->extra_index = value_tree;

        // Step 2: link leaf node content with actual leaf in the key ART.
        art_insert(art_key_prefix_tree_g, (unsigned char*)key,
        strlen(key), (void*)leaf_cnt);

    } else {// Case 2: The key has been indexed.

        //2. Update the value tree leaves.
        value_tree = leaf_cnt->extra_index;

        //3. Check if the value has been indexed.
        value_obj_locator_table =
        (hg_hash_table_t *)art_search(value_tree, (unsigned char *)str_value,
        strlen(str_value));
        ...
        // Update hash table on the key tree leaf_cnt
        name_prefix_obj_locator_table = leaf_cnt->obj_id_table;
        // put the obj_id into hash table again.
        // Currently, the pointer to actual metadata is unavailable.
        hg_hash_table_insert(name_prefix_obj_locator_table, obj_server_id, obj_locator);
