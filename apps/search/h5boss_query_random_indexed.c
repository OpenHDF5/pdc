...
/* *****************  PARTIAL QUERY STARTS **********************  */
    ...
    sprintf(p_query[0], "tag%d:NULL", n_query, n_query);//has tag
    sprintf(p_query[1], "tag%d*:NULL", n_query, n_query);//tag prefix match
    sprintf(p_query[2], "tag*%d:NULL", n_query, n_query);//tag suffix match
    sprintf(p_query[3], "*tag%d*:NULL", n_query, n_query);//tag infix match
    sprintf(p_query[4], "tag%d:%d", n_query, n_query);//tag value equals to
    sprintf(p_query[5], "tag%d:%d*", n_query, n_query);//tag value matches prefix
    sprintf(p_query[6], "tag%d:*%d", n_query, n_query);//tag value matches suffix
    sprintf(p_query[7], "tag%d:*%d*", n_query, n_query);//tag value matches infix
    ...
    for (n_round = 0; n_round < 11; n_round++) {
...
#else
        printf("Time to query %10d metadata objects with tag [%15s]: %.6f\n", n_res, p_query[n_round], ht_query_tag_sec);
#endif
